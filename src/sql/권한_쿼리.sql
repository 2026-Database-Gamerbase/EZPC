...
-- localhost : 같은 PC에서 접속 (로컬 테스트)
-- '%'       : 모든 외부 호스트에서 접속 (팀 테스트)
-- ============================================================
...
-- ------------------------------------------------------------
CREATE USER IF NOT EXISTS 'ezpc_auth'@'localhost' IDENTIFIED BY 'auth1234';
CREATE USER IF NOT EXISTS 'ezpc_auth'@'%'         IDENTIFIED BY 'auth1234';

GRANT SELECT, INSERT ON EZPC.pc_member TO 'ezpc_auth'@'localhost';
GRANT SELECT, INSERT ON EZPC.pc_member TO 'ezpc_auth'@'%';

GRANT SELECT ON EZPC.customer TO 'ezpc_auth'@'localhost'; -- 중복 로그인 체크용
...
--    리뷰 작성 등 사용자 업무만 수행
-- ------------------------------------------------------------
CREATE USER IF NOT EXISTS 'ezpc_user'@'localhost' IDENTIFIED BY 'user1234';
CREATE USER IF NOT EXISTS 'ezpc_user'@'%'         IDENTIFIED BY 'user1234';

-- 조회 가능 테이블
GRANT SELECT ON EZPC.pc_cafe        TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.pc_cafe        TO 'ezpc_user'@'%';

GRANT SELECT, UPDATE ON EZPC.pc_member TO 'ezpc_user'@'localhost';
GRANT SELECT, UPDATE ON EZPC.pc_member TO 'ezpc_user'@'%';
-- UPDATE: 퇴실 시 remain_time 갱신 (updateRemainTimeAfterUse)

GRANT SELECT ON EZPC.ticket         TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.ticket         TO 'ezpc_user'@'%';

GRANT SELECT ON EZPC.food           TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.food           TO 'ezpc_user'@'%';

GRANT SELECT ON EZPC.grade          TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.grade          TO 'ezpc_user'@'%';

GRANT SELECT ON EZPC.event_info     TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.event_info     TO 'ezpc_user'@'%';

GRANT SELECT ON EZPC.event_schedule TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.event_schedule TO 'ezpc_user'@'%';

GRANT SELECT, UPDATE ON EZPC.stock  TO 'ezpc_user'@'localhost'; -- 음식 주문 시 재고 차감
GRANT SELECT, UPDATE ON EZPC.stock  TO 'ezpc_user'@'%';         -- 음식 주문 시 재고 차감

GRANT SELECT ON EZPC.review         TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.review         TO 'ezpc_user'@'%';

GRANT SELECT ON EZPC.charge         TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.charge         TO 'ezpc_user'@'%';

GRANT SELECT ON EZPC.food_order     TO 'ezpc_user'@'localhost';
GRANT SELECT ON EZPC.food_order     TO 'ezpc_user'@'%';

-- 입퇴실 처리 (customer, use_log)
GRANT SELECT, INSERT, UPDATE, DELETE ON EZPC.customer TO 'ezpc_user'@'localhost';
GRANT SELECT, INSERT, UPDATE, DELETE ON EZPC.customer TO 'ezpc_user'@'%';

GRANT SELECT, INSERT, UPDATE ON EZPC.use_log TO 'ezpc_user'@'localhost';
GRANT SELECT, INSERT, UPDATE ON EZPC.use_log TO 'ezpc_user'@'%';

-- 리뷰 작성/수정/삭제
GRANT SELECT, INSERT, UPDATE, DELETE ON EZPC.review TO 'ezpc_user'@'localhost';
GRANT SELECT, INSERT, UPDATE, DELETE ON EZPC.review TO 'ezpc_user'@'%';

-- 음식 주문 삽입
GRANT SELECT, INSERT ON EZPC.food_order TO 'ezpc_user'@'localhost';
GRANT SELECT, INSERT ON EZPC.food_order TO 'ezpc_user'@'%';

-- 이용권 충전 프로시저 실행 (프로시저 내부에서 charge INSERT + 등급 갱신 처리)
GRANT EXECUTE ON PROCEDURE EZPC.charge_by_customer TO 'ezpc_user'@'localhost';
...
--    모든 테이블 접근 가능
-- ------------------------------------------------------------
CREATE USER IF NOT EXISTS 'ezpc_owner'@'localhost' IDENTIFIED BY 'owner1234';
CREATE USER IF NOT EXISTS 'ezpc_owner'@'%'         IDENTIFIED BY 'owner1234';

GRANT ALL PRIVILEGES ON EZPC.* TO 'ezpc_owner'@'localhost';
...
-- [확인 쿼리] 계정별 권한 조회
-- ============================================================
SHOW GRANTS FOR 'ezpc_auth'@'localhost';
SHOW GRANTS FOR 'ezpc_auth'@'%';
SHOW GRANTS FOR 'ezpc_user'@'localhost';
SHOW GRANTS FOR 'ezpc_user'@'%';
SHOW GRANTS FOR 'ezpc_owner'@'localhost';
